...
//import jsChessEngine from "js-chess-engine/dist/js-chess-engine.js";
//const jsChess = require('js-chess-engine');
import {fetchLichessDatabase, fetchLichessCloudEval} from "./libs/fetchLichess";
import { json } from "stream/consumers";
import clsx from "clsx";
import Worker from "../components/Worker";
/* import jsChessEngine from 'js-chess-engine'; */

{/* <iframe src="https://fritz.chessbase.com" className=" w-[760px] h-[480px]"></iframe> */}
 ...
 */
  //const gameEngine = new jsChess.Game();
  ...
  useEffect( () => {
    /* let lichessEval = await getCloudEval(game.fen());
    setEvaluation(lichessEval); */
    /* getCloudEval(game.fen()).then((res) => {
      console.log(res);
    }) */
  ...
  function calculateMoveGain(boardBefore: any, boardAfter: any) {
    const piecesValues = new Map([
      ['p', 1],
      ['r', 5],
      ['n', 3],
      ['b', 3],
    ...
    let boardBeforeScore = 0;
    let boardAfterScore = 0;

    //@ts-ignore
    boardBefore.forEach(line => {
      //@ts-ignore
      line.forEach(square => {
        //@ts-ignore
        if(square !== null) boardBeforeScore += piecesValues.get(square.type);
      })
    });

    //@ts-ignore
    boardAfter.forEach(line => {
      //@ts-ignore
      line.forEach(square => {
        //@ts-ignore
        if(square !== null) boardAfterScore += piecesValues.get(square.type);
      })
    });

    return boardBeforeScore - boardAfterScore;
  }

  function findBestMove(gameCopy: Chess) {
    ...
    possibleMoves.forEach(move => {
      const gameCopyDeeper = new Chess(gameCopy.fen());
      gameCopyDeeper.move(move);
      const points = calculateMoveGain(gameCopy.board(), gameCopyDeeper.board());
      if(points > bestMove.points){
        bestMove.notation = move;
  ...
  function chessMinMax(gameCopy: Chess, level: number, maxLevel: number) {
    ...
    possibleMoves.forEach(move => {
      const gameCopyDeeper = new Chess(gameCopy.fen());
      gameCopyDeeper.move(move);
      let points = calculateMoveGain(gameCopy.board(), gameCopyDeeper.board());

      points -= chessMinMax(gameCopyDeeper, level+1, maxLevel).points;
  ...
  function chessMinMax2(gameCopy: Chess, level: number, maxLevel: number) {
    if(level > maxLevel) return {
      notation: '',
      points: 0
    };
    ...
    possibleMoves.forEach(move => {
      //const gameCopyDeeper = new Chess(gameCopy.fen());
      const oldBoard = gameCopy.board();
      gameCopy.move(move);
      let points = calculateMoveGain(oldBoard, gameCopy.board());

      if(gameCopy.isCheckmate()) {
        points = 100;
      }else{
        points -= chessMinMax(gameCopy, level+1, maxLevel).points;
      }
  ...
  function chessEngine() {
    ...
    console.time('Engine');
    bestMove = chessMinMax2(gameCopy, 1, 2);
    console.timeEnd('Engine');
    console.log(bestMove);
    if(bestMove.notation !== '') {
      game.move(bestMove.notation);
      setCount(0);
    } else {
  ...
  function enginesComparaison(depth: number) {
    // 1. e4 e5 2. Nf3 Nc6 3. Bb5 Bc5 4. c3 Nf6 5. d4 exd4 6. e5 Ne4 7. O-O dxc3 8. Qd5
    const newGame1 = new Chess('r1bqk2r/pppp1ppp/2n5/1BbQP3/4n3/2p2N2/PP3PPP/RNB2RK1 b kq - 1 8');
    const newGame2 = new Chess('r1bqk2r/pppp1ppp/2n5/1BbQP3/4n3/2p2N2/PP3PPP/RNB2RK1 b kq - 1 8');
    ...
    console.log(newGame1.ascii());

    console.time('Engine');
    bestMove = chessMinMax(newGame1, 1, depth);
    console.timeLog('Engine', bestMove); //469s

    console.time('Engine2');
    bestMove = chessMinMax2(newGame2, 1, depth);
    console.timeLog('Engine2', bestMove); //459s
  }

  function methodsComparaison() {
    ...
    console.time("isCheckmate");
    for(let i = 0; i < 10000; i++){
      newGame.isCheckmate();
    }
    console.timeEnd("isCheckmate");

    console.time("moveUndo");
    for(let i = 0; i < 10000; i++){
      newGame.move('e4');
      newGame.undo();
    }
    console.timeEnd("moveUndo");

    console.time("newChessMove");
    for(let i = 0; i < 10000; i++){
      const newGame2 = new Chess();
      newGame2.move('e4');
    }
    console.timeEnd("newChessMove");
  }

  function test() { 
    enginesComparaison(2);
  }

  async function getCloudEval(fen: string) {
    let cloudEval = await fetchLichessCloudEval(fen);
    //engine.evaluatePosition(fen, 16);
  ...
  return (
    <div className="flex flex-col justify-center items-center bg-cyan-900 h-full w-full overflow-auto" >
        ...
        <div className=" h-20 w-full flex flex-col justify-center items-center">
          <div className=" text-white" >{`Eval: ${evaluation}`}</div>
